...
1. **Minimum distance**: Must end at least **250m** from starting position (prevents gaming the Moved DRM for Indirect Fire; if you want to stay within 250m, use [[hold|Hold]] instead)
2. **Blocking**: Enemy FE counters block movement — cannot move through them (but does not block placement at an [[assault|Assault Location]])
3. **End position**:
   - Cannot overlap counters with another friendly FE (unless [[stacking|Mounting/Embedding]])
   - Cannot end within **250m** of an enemy FE (unless on the other side of a River)

> [!note]
> If these restrictions can't be met due to EW Chit effects or Suppression/Disruption received during the Move, the FE should be positioned as close as possible while meeting the criteria.

## No-Go Terrain

- **Dense Wood**: No-Go for Tracked and Wheeled (can only enter if on a Road; may partially enter < 50% of counter)
- **Rivers**: No-Go for all FE types (must cross at a Bridge)
...
- May coexist with an enemy UAS
- Begins off-map; placed on first Move Action

## Moving Off-Map

An FE moved off the map is **removed from the game**. It does not count as Eliminated but cannot re-enter play.
...
- [[overwatch#Reactive Fire|Reactive Fire]] — opponents with Overwatch can fire at moving FEs
